...
import { api, Config, Account } from './api'
...
const updateStorage = (url: ServerUrl) =>
  url
    ? AsyncStorage.setItem(storageKey, JSON.stringify(url))
    : AsyncStorage.removeItem(storageKey)
const urlToStr = (url: ServerUrl) =>
...
export const config: Observable<Config | undefined> = serverUrl
  .switchMap(
    (u, idx) =>
      idx === 0
        ? Observable.of(undefined) // 0-th / default - for sure wrong
        : Observable.defer(() => api.config(urlToStr(u)))
            ...
            .retryWhen(es =>
              es.do(() => errorSub.next('CANNOT_CONNECT')).delay(250)
            )
  )
  .shareReplay(1)
...
export const accounts: Observable<Account[]> = Observable.combineLatest(
  contractsAccount,
  config
)
  .switchMap(
...
export const combined = Observable.combineLatest(
  ...
  (url, config, contractsAccount, accounts) => ({
    ...
    contractsAccount,
    accounts,
    isConfigOk: contractsAccount && accounts.length > 0,
  })
)
